...
for(int i=0; i<(int)node.size()-1;i++){
    for(int k=0;k<(int)node[i+1].size();k++){
        ...
        node[i+1][k] = (float)activation(sum);  // make sure it works for gpu
    }
}
...
bias[n][j]-=Lr*2*(node[n][j]-e[j])*d_activation(node[n][j]);
}

// make every thing in one for cycle so that can be trasforme into __gloabal
for(int i=n; i>0;i--){
    for(int k=0;k<(int)node[i-1].size();k++){
        d[i-1][k]=0;
        for(int j=0;j<(int)node[i].size();j++){
...
int main(){
	...
	int M;
	cin >> N >> M;
    size_t size_w = N * M * M * sizeof(float);
    size_t size_n = N * M * sizeof(float);
    //alocate cpu mem
    
    //
	vector<vector<vector<float>>> weights(N+2, vector<vector<float>>(M, vector<float>(M,0.05)));
...
for(int i=0;i<data.size();i++){
    ...
    Train(weights,nodes,biases,deltas,waves,wform,amp,100);
    // free cpu mem
	return 0;
}
